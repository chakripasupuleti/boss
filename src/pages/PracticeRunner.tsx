...
const interpolateTemplate = (template: string, vars: Record<string, number>): string => {
  ...
  switch (power_mod_4) {
    case 0:
      final_answer = `${1 + constant}`;
      break;
    case 1:
      final_answer = `i + ${constant}`;
      break;
    case 2:
      final_answer = `${-1 + constant}`;
      break;
  ...
  // LCM greatest length
  const length1 = vars.length1 ?? 14,
    length2 = vars.length2 ?? 21,
    length3 = vars.length3 ?? 35;
...
const practiceQuestions: Record<string, Record<string, QuestionData>> = {
  ...
  "lcm-hcf": {
    m1: {
      ...
      hint: "Use prime factorization take the lowest powers of common primes for the HCF and the highest powers for the LCM.",
    ...
    m2: {
      ...
      variables: {
        frac1_num: { min: 2, max: 15, default: 7 },
        frac1_den: { min: 2, max: 15, default: 9 },
        frac2_num: { min: 2, max: 70, default: 63 },
        frac2_den: { min: 2, max: 15, default: 8 },
        frac3_num: { min: 2, max: 15, default: 9 },
        frac3_den: { min: 2, max: 15, default: 7 },
      },
    },
    m3: {
    ...
    m4: {
      ...
      variables: {
        length1: { min: 10, max: 30, default: 14 },
        length2: { min: 15, max: 35, default: 21 },
        length3: { min: 20, max: 65, default: 35 },
      },
    },
  },
  percentages: {
    ...
    m2: {
      ...
      variables: {
        numerator1: { min: 1, max: 9, default: 3 },
        denominator1: { min: 2, max: 12, default: 4 },
        numerator2: { min: 1, max: 7, default: 1 },
        denominator2: { min: 2, max: 12, default: 2 },
      },
    },
    m3: {
    ...
    m4: {
      ...
      variables: {
        percent1: { min: 10, max: 90, default: 20 },
        percent2: { min: 10, max: 90, default: 25 },
        number: { min: 10, max: 999, default: 300 },
      },
    },
  },
};
...
export default function PracticeRunner() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">The requested practice question could not be loaded.</p>
        </div>
      </div>
    );
  }
  ...
  const timeColor = timeLeft > 25 ? "bg-primary" : timeLeft > 10 ? "bg-energy" : "bg-destructive";
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`font-bold text-lg ${
                    timeLeft <= 10 ? "text-destructive" : timeLeft <= 25 ? "text-energy" : "text-primary"
                  }`}
                >
                  {timeLeft}s
                </span>
                <Button variant="ghost" size="icon" onClick={resetTimer} disabled={!isTimerActive}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="p-4 rounded-lg bg-muted/50 text-lg">
                  <MathRenderer>{interpolateTemplate(question.stem, variables)}</MathRenderer>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  {isCorrect !== null && (
                    <Badge variant={isCorrect ? "secondary" : "destructive"} className="px-4 py-2 text-sm">
                ...
                {isCorrect === true && (
                  <div className="p-4 rounded-lg bg-success/10 border border-success/20">
                    ...
                    <p className="text-sm mt-1">
                      You can now adjust the variables below to practice with different variations.
                    </p>
                  </div>
                )}
                ...
                {isCorrect === false && (
                  <div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20">
                    ...
                    <p className="text-sm mt-1">
                      The correct answer is: <strong>{calculateDynamicAnswer(model || "", variables)}</strong>
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            {isDynamic && question.variables && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">Practice Variations</CardTitle>
                  <CardDescription>Adjust the parameters below to generate new question variations</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {Object.entries(question.variables).map(([key, config]) => (
                    <div key={key} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label className="capitalize">{key.replace("_", " ")}</Label>
                        <span className="text-sm font-medium">{variables[key]}</span>
                      </div>
                      <Slider
              ...
              <CardContent>
                {showHint ? (
                  <div className="p-3 rounded-lg bg-highlight/10 border border-highlight/20 max-h-52 overflow-y-auto">
                    <MathRenderer className="text-sm">{interpolateTemplate(question.hint, variables)}</MathRenderer>
                  </div>
                ) : (
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    {showExplanation ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
                    Explanation
                  </span>
                  <Button
                    variant="ghost"
              ...
              <CardContent>
                {showExplanation ? (
                  <div className="space-y-3">
                    {isLoadingAI && !aiStreamDone ? (
                      <div className="space-y-3">
                        ...
                        {aiStreamText && (
                          <div className="p-3 rounded-md bg-muted/30 text-sm max-h-80 overflow-y-auto">
                            <pre className="whitespace-pre-wrap font-sans text-foreground/70">{aiStreamText}</pre>
                          </div>
                        )}
                      </div>
                    ) : aiExplanation ? (
                      <div className="p-3 rounded-lg bg-accent/10 border border-accent/20 max-h-80 overflow-y-auto">
                        ...
                        <MathRenderer className="text-sm leading-relaxed">{aiExplanation}</MathRenderer>
                      </div>
                    ) : explanationError ? (
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    ...
                    <p className="text-sm">
                      {isCorrect === null ? "Submit your answer to view explanation" : "Click Show to view explanation"}
                    </p>
                  </div>
                )}
              </CardContent>
